...
defmodule SublistTest do
  ...
  property "The empty list is always a sublist of any other list" do
    elem = one_of([integer(), binary()])
    check all list <- list_of(elem, min_length: 1) do
      assert Sublist.compare([], list) == :sublist
    end
  end

  property "Any list is a superlist of the empty list" do
    elem = one_of([integer(), binary()])
    check all list <- list_of(elem, min_length: 1) do
      assert Sublist.compare(list, []) == :superlist
    end
  end
